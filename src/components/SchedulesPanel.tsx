...
import ScheduleModal, { type ScheduleFormData } from "./SchedulesModal";
...
type Schedule = {
  ...
  nextRunAt: string | null;  // Make sure this exists
  consecutiveEmptyRuns?: number;
  emailsPerCompany?: number;  // Add this if missing
  disabledReason?: string;
  lastRunStats?: { found: number; sent: number; failed: number };
  sender: { studioName: string; yourName: string; baseOffer: string };
  providers?: SearchProvider[];  // Add this if missing
};
                    ...
                    <td className="px-3 py-3 text-xs">
                      {s.lastRunStats ? (
                        <span>
                          ...
                          {s.lastRunStats.failed > 0 && (
                            <span className="text-red-300 ml-1">({s.lastRunStats.failed} err)</span>
                          )}
                        </span>
                      ) : (
                        <span className="text-slate-500">—</span>
